...
namespace Test
{
	TEST_CLASS(ThreadSmall)
	{
	public:

		TEST_METHOD(small_size_thread_small)
		{
			Graph<double> *graph = new Graph<double>(SMALL_SIZE);
			Dijkstra<double> *dijkstra = new Dijkstra<double>(graph);
			dijkstra->run(0, SMALL_THREAD);
			delete graph;
			delete dijkstra;
		}
		TEST_METHOD(middle_size_thread_small)
		{
		...
		TEST_METHOD(big_size_thread_small)
		{
